...
from rkd.core.exception import DeclarationException, ParsingException


class TestApiParsing(BasicTestingCase):
    def test_parse_imports_successful_case_single_task(self):
        imported = SyntaxParsing.parse_imports_by_list_of_classes(['rkd.core.standardlib.jinja.RenderDirectoryTask'])

        self.assertEqual(':j2:directory-to-directory', imported[0].to_full_name())

    def test_parse_imports_successful_case_module(self):
        imported = SyntaxParsing.parse_imports_by_list_of_classes(['rkd.core.standardlib.jinja'])
    ...
    def test_parse_imports_wrong_class_type_but_existing(self):
        def test():
            SyntaxParsing.parse_imports_by_list_of_classes(['rkd.core.exception.ContextException'])

        self.assertRaises(DeclarationException, test)

    def test_parse_imports_cannot_import_non_existing_class(self):
        def test():
            SyntaxParsing.parse_imports_by_list_of_classes(['rkd.core.standardlib.python.WRONG_NAME'])

        self.assertRaises(ParsingException, test)

    def test_parse_imports_importing_whole_module_without_submodules(self):
        imported = SyntaxParsing.parse_imports_by_list_of_classes(['rkd.pythonic'])

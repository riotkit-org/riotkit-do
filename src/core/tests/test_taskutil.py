...
for i in range(0, 1024 * 128):
        ...
        for i in range(1, 5):
            for std_redirect in ['', '>&2']:
                ...
                with io.capture_descriptors(stream=out, enable_standard_out=False):
                    try:
                        task.sh(''' 
                            python3 -m rkd.core --silent :sh -c 'echo "Bartolomeo Vanzetti" ''' + std_redirect + '''; exit 127'
                        ''')
    ...
    def test_non_interactive_session_returns_output(self):
        ...
        with io.capture_descriptors(stream=out, enable_standard_out=False):
            task.sh(''' true | python3 -m rkd.core --silent :sh -c 'echo "Strajk Kobiet! Jebac PiS!"' ''')
    ...
    def test_full_command_is_shown_only_in_debug_output_level(self):
        ...
        with io.capture_descriptors(stream=out, enable_standard_out=False):
            ...
            with self.subTest('NORMAL output level'):
                try:
                    task.sh('python3 -m rkd.core :sh -c "exit 5"')
            ...
            with self.subTest('DEBUG output level'):
                try:
                    task.sh('python3 -m rkd.core -rl debug :sh -c "exit 5"')
    ...
    def test_sh_3rd_depth_rkd_calls(self):
        ...
        for std_redirect in ['', '>&2']:
            ...
            with io.capture_descriptors(stream=out, enable_standard_out=False):
                try:
                    task.sh(''' 
                        python3 -m rkd.core --silent :sh -c 'python -m rkd.core --silent :sh -c \'echo "Nicola Sacco" ''' + std_redirect + '''; exit 1\''
                    ''')
                except subprocess.CalledProcessError:
                    pass
